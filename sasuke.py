...
API_ID = 24339943 # Replace with your actual API ID
API_HASH = "d64cb988da55d5ccf2115a99bd6819ea" # Replace with your actual API HASH
...
async def scrape_messages(client, channel_username, limit, start_number=None):
    ...
    async for message in user_app.search_messages(channel_username):
        if count >= limit:
            break
...
async def scr_cmd_func(client, message, channel_username, limit, start_number=None):
    temporary_msg = bot.reply_to(message, "<b>Scraping in progress wait.....</b>", parse_mode="HTML")
    scrapped_results = await scrape_messages(user_app, channel_username, limit, start_number)
    unique_messages, duplicates_removed = remove_duplicates(scrapped_results)
    if unique_messages:
        file_name = f"x{len(unique_messages)}_{channel_username.replace(' ', '_')}.txt"
        ...
        with open(file_name, 'rb') as f:
            caption = (
                ...
                f"<b>━━━━━━━━━━━━━━━━</b>\n"
                f"<b>Card-Scrapper By: Goku</b>"
            )
            bot.delete_message(message.chat.id, temporary_msg.message_id)
            bot.send_document(message.chat.id, f, caption=caption, parse_mode="HTML")
        os.remove(file_name)
    else:
        bot.delete_message(message.chat.id, temporary_msg.message_id)
        bot.send_message(message.chat.id, "<b>Sorry Bro ❌ No Credit Cards Found</b>", parse_mode="HTML")

@bot.message_handler(func=lambda message: message.text.startswith("/scr") or message.text.startswith(".scr"))
def scr_cmd(message):
    user_id = str(message.from_user.id)
    ...
    if len(args) < 2 or len(args) > 3:
        bot.reply_to(message, "<b>⚠️ Provide channel username and amount to scrape</b>", parse_mode="HTML")
        return
    channel_identifier = args[0]
    limit = int(args[1])
    max_lim = ADMIN_LIMIT
    if limit > max_lim:
        bot.reply_to(message, f"<b>Sorry Bro! Amount over Max limit is {max_lim} ❌</b>", parse_mode="HTML")
        return
    ...
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(scr_cmd_func(bot, message, channel_username, limit, start_number))
        loop.close()
    except Exception as e:
        bot.reply_to(message, f"<b>Hey Bro! 🥲 Error during scraping: {e} ❌</b>", parse_mode="HTML")
...
/scr &lt;channel_username&gt; &lt;amount&gt; - Scrape CCs from a telegram channel.
...
This bot is designed for card checking, generation and scraping from telegram channels.
...
async def on_startup():
    await user_app.start() # try to start user_app here to load session initially
    logop = f'''━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━bot by @GOKUOFFICIALREAL_BOT started successfully ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'''
    print(logop)
